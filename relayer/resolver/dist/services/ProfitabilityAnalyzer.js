...
class ProfitabilityAnalyzer {
    constructor(config) {
        this.logger = (0, Logger_1.createLogger)("ProfitabilityAnalyzer");
        this.config = config;
    }
    /**
     * Analyze profitability of a Fusion+ order
     */
    async analyzeProfitability(fusionOrder) {
        try {
            ...
            const totalCosts = Object.values(costs).reduce((sum, cost) => sum + parseFloat(cost), 0);
     ...
     */
    async fetch1inchQuote(fusionOrder) {
        ...
        const response = await (0, utils_1.retryAsync)(async () => {
        ...
        return {
            ...
            protocols: data.protocols?.map((p) => p.name) || [],
        };
    }
    /**
     * Calculate various costs involved in the trade
     */
    async calculateCosts(fusionOrder) {
        ...
        const safetyDeposit = parseFloat((0, utils_1.formatEthAmount)(fusionOrder.srcSafetyDeposit)) +
            parseFloat((0, utils_1.formatEthAmount)(fusionOrder.dstSafetyDeposit));
        return {
     ...
     */
    async estimateEvmGasCost(fusionOrder) {
        try {
            ...
            this.logger.warn("Failed to estimate EVM gas cost, using default:", (0, utils_1.extractErrorMessage)(error));
     ...
     */
    async estimateAptosGasCost(fusionOrder) {
        try {
            ...
            this.logger.warn("Failed to estimate Aptos gas cost, using default:", (0, utils_1.extractErrorMessage)(error));
    ...
    async checkMarketConditions() {
        try {
            ...
            this.logger.warn("Failed to check market conditions:", (0, utils_1.extractErrorMessage)(error));
